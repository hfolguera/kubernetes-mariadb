...
DATABASES=`kubectl exec -i -n $NS $POD -- /usr/bin/mariadb -u root --password=${PASS} -e "SHOW DATABASES;" | grep -v "+-" | grep -v Database | grep -v information_schema | grep -v performance_schema | grep -v Warning | awk '{print $1}'`
if [ $? -eq 0 ]
then
...
do
    ...
    kubectl exec -it -n $NS $POD -- /usr/bin/mysqldump -u root --password=${PASS} $DB > $DEST_PATH/mariadb_${DB}_backup_${DATE}.sql
    if [ $? -ne 0 ]
    then
